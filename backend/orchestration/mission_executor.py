...
class MissionExecutor:
    ...
    async def list_tenant_missions(self, tenant_id: str, limit: int = 50, offset: int = 0) -> List[dict]:
    ...
    async def execute_mission(self, mission_id: str, goal: str, tenant_id: str, user_id: str, budget: float = None, name: str = "unnamed") -> dict:
        ...
        try:
            ...
            # Phase 1: Planning
            llm = self.llm_factory.create_llm(provider=settings.COMMANDER_PROVIDER, model=settings.COMMANDER_MODEL)
            plan_response = await llm.ainvoke(f"Create a 3-step plan for: {goal}. Respond with only the steps, one per line.")
            ...
            for i, step in enumerate(steps):
                ...
                await self.marketplace.charge(tenant_id, "executor", 1.0, "llm_call", mission_id=mission_id)
                total_cost += 1.0
                
                logger.info(f"Calling LLM for step {i+1}...")
                exec_llm = self.llm_factory.create_llm(provider=settings.COMMANDER_PROVIDER, model=settings.COMMANDER_MODEL)
